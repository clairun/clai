...
mod tests {
    use super::*;
    use crate::assistant::compaction::COMPACTION_METADATA_SOURCE;
    use crate::assistant::types::AssistantCompaction;
    use crate::assistant::types::CompactionStatus;
    use crate::assistant::types::CompactionStrategy;
    ...
    /// Regression (GLM/Z.ai 400 "The messages parameter is illegal"): after a
    /// compaction whose cut fell after the last user message, the tail is all
    /// assistant/tool messages. The history is built through the real
    /// compaction seam, which role-flips the persisted summary to User, so
    /// the final request — system prompt prepended like `run_assistant_loop`
    /// does — always contains a user message instead of the old user-less
    /// shape. Reverting the role flip in `provider_bound_summary` fails here.
    #[test]
    fn normalize_keeps_post_compaction_history_user_led() {
        let mut old_user = user_message("do the work");
        old_user.id = "old-0".to_string();
        let mut cut = assistant_message_with_content(vec![ContentPart::ToolUse {
            tool_call_id: "call_a".into(),
            tool_name: "bash_exec".into(),
            arguments: serde_json::json!({}),
        }]);
        cut.id = "cut".to_string();
        let summary = AssistantMessage {
            id: "summary".to_string(),
            session_id: "session".to_string(),
            role: MessageRole::System,
            content: vec![ContentPart::Text {
                text: "Conversation summary generated by CLAI compaction.".to_string(),
            }],
            created_at: 0,
        ...
        let mut tail_assistant = assistant_message_with_content(vec![ContentPart::ToolUse {
            tool_call_id: "call_b".into(),
            tool_name: "bash_exec".into(),
            arguments: serde_json::json!({}),
        }]);
        tail_assistant.id = "tail-asst".to_string();
        let mut tail_result = tool_message("call_b");
        tail_result.id = "tail-res".to_string();
        let messages = vec![old_user, cut, summary, tail_assistant, tail_result];
        let compaction = AssistantCompaction {
            ...
            source_to_message_id: Some("cut".to_string()),
        ...
        let compacted =
            compaction::provider_history_messages_with_compaction(&messages, Some(&compaction));
        let normalized = normalize_history_for_provider(&compacted);
        let mut provider_messages = vec![ProviderInputMessage {
            role: MessageRole::System,
            content: vec![ContentPart::Text {
        ...
        assert_eq!(
            ...
            "Z.ai rejects a messages array with no user message"
        );
    }
