...
/// on `exec resume`).
///
/// Deliberately omits `excludeTurns`, so the app server restores the thread
/// with its turns included — the resumed run sees the same replayed
/// notifications as a fresh one.
...
mod tests {
    ...
    fn thread_resume_disables_shell_tool_and_carries_mcp_config() {
        ...
        // Turns are never excluded on resume: see the doc comment above.
