...
const Fleet = () => {
  ...
  const handleCloneConfig = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError('');
      await loadWorkspaces();
      // Select the fresh clone so the user lands on it ready to send a message.
                ...
                <div className={styles.cardHeader}>
                  ...
                  <div className={styles.cardHeaderActions}>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleCloneConfig(ws.id);
                      }}
                      disabled={cloneBusyId === ws.id}
                      title={cloneBusyId === ws.id ? 'Cloning…' : 'Clone config to a new workspace (no history)'}
                      aria-label="Clone workspace config"
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
